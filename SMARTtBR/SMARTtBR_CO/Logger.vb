...
Public Class Logger
    Private Shared Sub Info(ByVal info As Object)
        ...
        Dim folderName As String = ConfigurationManager.AppSettings("LOG_FOLDER").ToString()
        Dim fileName As String = ConfigurationManager.AppSettings("LOG_FILE").ToString()
        Dim dir As DirectoryInfo = New DirectoryInfo(folderName)
        'Check for existence of logger file            
        If File.Exists(fileName) Then
            Try
                Dim fs As FileStream = New FileStream(folderName + "\" + fileName, FileMode.Append, FileAccess.Write)
