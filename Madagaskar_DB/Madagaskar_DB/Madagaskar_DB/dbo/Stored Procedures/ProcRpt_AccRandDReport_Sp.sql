...
-- Exec ProcRpt_AccRandDReport_Sp 1,7002,22,1,'<NewDataSet />','<NewDataSet />','02/28/2017','02/28/2017'
CREATE Procedure [dbo].[ProcRpt_AccRandDReport_Sp] 
(
	@CompanyCode		As Int,
	@MMID				As Int,
	@RptID				As Int, 
...
Begin
	...
	Declare @Qry				NVarchar(Max),
			...
			@ReportHeading		Varchar(250) = 'ACCOUNT RandD REPORT',
			@NoOfCopies			Int	= 1
	...
	IF @RptID = 21
	Begin
		Insert InTo #Ledger(SLNO,UNName,ID,Date,AccGroup,AccGroupName,AccCode,AccName,Remarks,Amount,Debit,Credit)
	...
	Begin
		Insert InTo #Ledger(SLNO,UNName,ID,Date,AccGroup,AccGroupName,AccCode,AccName,Remarks,Amount,Debit,Credit)
		SELECT ROW_NUMBER() OVER(ORDER BY AccCode,Date)SLNO,UNName,ROW_NUMBER() OVER(ORDER BY AccCode,Date) ID,
				Date,AccGroup,AccGroupName,AccCode,AccName,Remarks,Amount,Debit,Credit
		FROM 
		(
			SELECT ROW_NUMBER() OVER(ORDER BY AccCode,Date)SLNO,'ALL' UNName,MAX(ID) ID,Date,AccGroup,AccGroupName,AccCode,AccName,
				MAX(Remarks) Remarks,SUM(Amount) Amount,SUM(Debit) Debit,SUM(Credit) Credit
			FROM #TrDetails M GROUP BY AccGroup,AccGroupName,AccCode,AccName,Date
		)As X ORDER BY AccGroup,AccCode,Date
	End
