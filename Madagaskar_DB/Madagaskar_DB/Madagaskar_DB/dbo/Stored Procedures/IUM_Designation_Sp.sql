...
-- Exec IUM_Designation_Sp
CREATE PROCEDURE [dbo].[IUM_Designation_Sp]
(
...
BEGIN
 ...
 Begin Try
	...
	IF @Entry_Mode ='NEW'
	BEGIN 
		Exec GetNextID_Sp 'Designation_P_Tbl','Des_ID',@NextID  OUTPUT 
		SET @Des_ID = @NextID 
	
		INSERT INTO dbo.Designation_P_Tbl
		(
			Des_ID,
			Des_Name,
			Active_Status,
	...
	BEGIN
		...
		Exec dbo.Proc_LogCurrent_ModifyDetails_Sp 'Designation_P_Tbl','Des_ID',@Des_ID

		UPDATE dbo.Designation_P_Tbl SET  
			Des_Name = @Des_Name ,
		...
		Exec dbo.IUM_LogMaster_Sp @LM_MMID = @MM_ID, @LM_MID = @Des_ID, @Maker_ID = @Updater_ID, @Making_Time = @Updating_Time ,@LogDetailsXML	= @LogDetailsXML
	END
