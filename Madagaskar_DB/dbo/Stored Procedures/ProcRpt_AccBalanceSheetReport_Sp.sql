...
-- Exec ProcRpt_AccBalanceSheetReport_Sp 1,7007,51,1,'<NewDataSet />','02/28/2017'
CREATE Procedure [dbo].[ProcRpt_AccBalanceSheetReport_Sp] 
(
	@CompanyCode		As Int,
	@MMID				As Int,
	@RptID				As Int, 
...
Begin
	...
	Declare @Qry				NVarchar(Max),
			...
			@ReportHeading		Varchar(250) = 'ACCOUNT BALANCE SHEET REPORT',
			@NoOfCopies			Int	= 1 

	Declare @OpeningStock		Numeric(18,2)= 0,
			@ClosingStock		Numeric(18,2)= 0

	Declare @FinYear_From		Date= Case When Month(@AsOnDate)< 4 Then Cast('04/01/'+ Cast((Year(@AsOnDate)-1) as varchar(4))AS Date) Else Cast('04/01/'+ Cast((Year(@AsOnDate)) as varchar(4))AS Date)End,
			@FinYear_To			Date= Case When Month(@AsOnDate)< 4 Then Cast('03/31/'+ Cast((Year(@AsOnDate)) as varchar(4))AS Date) Else Cast('03/31/'+ Cast((Year(@AsOnDate)+1) as varchar(4))AS Date)End
	...
	Set @Qry = 'Insert InTo #TrDetails(SLNO,UNID,UNName,ID,Date,AccGroup,AccGroupName,AccGroupSeqNo,AccCode,AccName,AccSeqNo,AccALIE,AccPosition,Remarks,TranType,Debit,Credit,Amount)
	...
	(
		...
		FROM AccOpBal_Vw M WHERE Active_Status = ''Y'' AND Acc_ALIE IN(''A'',''L'') AND Acc_OpDate = DateAdd(Day,-1,''' + CAST(@FinYear_From as varchar(12)) + ''')  
			AND Exists(Select 1 From ##UnitFilter_Tbl U where M.Acc_UNID = U.UN_ID) AND Cmp_Code =' + Cast(@CompanyCode As Varchar(20)) +'  
		...
		FROM AccLedgerDetails_Vw M WHERE Active_Status = ''Y'' AND Cmp_Code =' + Cast(@CompanyCode As Varchar(20)) + ' 
			AND Acc_TrDate Between ''' + CAST(@FinYear_From as varchar(12)) + ''' AND ''' + CAST(@AsOnDate as varchar(12)) + ''' 
	...
	If OBJECT_ID('TempDB.dbo.#PandL_Debit') Is Not Null Drop Table #PandL_Debit
	Create Table #PandL_Debit
	(
		[SLNO]				[INT] IDENTITY(1,1),
		[AccGroup]			[bigint]		NULL,
		[AccGroupName]		[varchar](50)	NULL, 
	...
	INSERT INTO #PandL_Debit(AccGroup,AccGroupName,Exp_Liab)
	...
	INSERT INTO #PandL_Debit(AccGroup,AccGroupName,AccGroupSeqNo,AccCode,AccName,AccSeqNo,Balance,GroupTotal,Exp_Liab)
	...
	INSERT INTO #PandL_Debit(AccGroup,AccGroupName,GroupTotal,Exp_Liab)
	SELECT AccGroup,AccGroupName,SUM(Balance) GroupTotal,Exp_Liab FROM #PandL_Debit GROUP BY AccGroup,AccGroupName,Exp_Liab ORDER BY AccGroupName 
	...
	INSERT INTO #PandL_Statement_Debit(AccGroup,AccGroupName,AccGroupSeqNo,AccCode,AccName,AccSeqNo,Balance,GroupTotal,Exp_Liab)
	SELECT AccGroup,AccGroupName,AccGroupSeqNo,AccCode,AccName,AccSeqNo,Balance,GroupTotal,Exp_Liab FROM #PandL_Debit ORDER BY AccGroupName,SLNO
	-- ========================================PROCESS P AND L DEBIT(EXPENSE)SIDE START=====================================================================================================================
	...
	If OBJECT_ID('TempDB.dbo.#PandL_Credit') Is Not Null Drop Table #PandL_Credit
	Create Table #PandL_Credit
	(
		[SLNO]				[INT] IDENTITY(1,1),
		[AccGroup]			[bigint]		NULL,
		[AccGroupName]		[varchar](50)	NULL, 
	...
	INSERT INTO #PandL_Credit(AccGroup,AccGroupName,Inc_Asset)
	...
	INSERT INTO #PandL_Credit(AccGroup,AccGroupName,AccGroupSeqNo,AccCode,AccName,AccSeqNo,Balance,GroupTotal,Inc_Asset)
	...
	INSERT INTO #PandL_Credit(AccGroup,AccGroupName,GroupTotal,Inc_Asset)
	SELECT AccGroup,AccGroupName,SUM(Balance) GroupTotal,Inc_Asset FROM #PandL_Credit GROUP BY AccGroup,AccGroupName,Inc_Asset ORDER BY AccGroupName
	...
	SELECT AccGroup,AccGroupName,AccGroupSeqNo,AccCode,AccName,AccSeqNo,Balance,GroupTotal,Inc_Asset FROM #PandL_Credit ORDER BY AccGroupName, SLNO
	...
	SET @TotalIncome = ISNULL((SELECT SUM(GroupTotal) FROM #PandL_Statement_Credit WHERE Inc_Asset ='I'),0)
	SET @TotalExp = ISNULL((SELECT SUM(GroupTotal) FROM #PandL_Statement_Debit WHERE Exp_Liab = 'E'),0)

	SET @PORL = @TotalIncome-@TotalExp

	-- ========================================PROCESS P AND L FINAL=====================================================================================================================

	-- ========================================PROCESS B S DEBIT(LIABILITY)SIDE START======================================================================================================
	If OBJECT_ID('TempDB.dbo.#BS_Statement_Debit') Is Not Null Drop Table #BS_Statement_Debit
	Create Table #BS_Statement_Debit
	(
		[SLNO]				[INT] IDENTITY(1,1),
		[AccGroup]			[bigint]		NULL,
		[AccGroupName]		[varchar](50)	NULL, 
	...
	If OBJECT_ID('TempDB.dbo.#BS_Debit') Is Not Null Drop Table #BS_Debit
	Create Table #BS_Debit
	(
		[SLNO]				[INT] IDENTITY(1,1),
		[AccGroup]			[bigint]		NULL,
		[AccGroupName]		[varchar](50)	NULL, 
	...
	INSERT INTO #BS_Debit(AccGroup,AccGroupName,Exp_Liab)
	SELECT AccGroup,AccGroupName,'L' Exp_Liab FROM #TrialBal_Tbl WHERE AccALIE IN ('A', 'L') GROUP BY AccGroup,AccGroupName HAVING SUM(Credit)>0

	INSERT INTO #BS_Debit(AccGroup,AccGroupName,AccGroupSeqNo,AccCode,AccName,AccSeqNo,Balance,GroupTotal,Exp_Liab)
	SELECT AccGroup,AccGroupName,AccGroupSeqNo,AccCode,AccName,AccSeqNo,Credit Balance,0 GroupTotal,'L' Exp_Liab
	FROM #TrialBal_Tbl WHERE AccALIE IN ('A', 'L') AND Credit > 0

	INSERT INTO #BS_Debit(AccGroup,AccGroupName,GroupTotal,Exp_Liab)
	SELECT AccGroup,AccGroupName,SUM(Balance) GroupTotal,Exp_Liab FROM #BS_Debit GROUP BY AccGroup,AccGroupName,Exp_Liab ORDER BY AccGroupName 

	INSERT INTO #BS_Statement_Debit(AccGroup,AccGroupName,AccGroupSeqNo,AccCode,AccName,AccSeqNo,Balance,GroupTotal,Exp_Liab)
	SELECT AccGroup,AccGroupName,AccGroupSeqNo,AccCode,AccName,AccSeqNo,Balance,GroupTotal,Exp_Liab FROM #BS_Debit ORDER BY AccGroupName,SLNO
	-- ========================================PROCESS B S DEBIT(LIABILITY)SIDE START=====================================================================================================================

	-- ========================================PROCESS B S CREDIT(ASSET)SIDE START=====================================================================================================================
	If OBJECT_ID('TempDB.dbo.#BS_Statement_Credit') Is Not Null Drop Table #BS_Statement_Credit
	Create Table #BS_Statement_Credit
	(
		[SLNO]				[INT] IDENTITY(1,1),
		[AccGroup]			[bigint]		NULL,
		[AccGroupName]		[varchar](50)	NULL, 
	...
	If OBJECT_ID('TempDB.dbo.#BS_Credit') Is Not Null Drop Table #BS_Credit
	Create Table #BS_Credit
	(
		[SLNO]				[INT] IDENTITY(1,1),
		[AccGroup]			[bigint]		NULL,
		[AccGroupName]		[varchar](50)	NULL, 
	...
	INSERT INTO #BS_Credit(AccGroup,AccGroupName,Inc_Asset)
	SELECT AccGroup,AccGroupName,'A' Inc_Asset FROM #TrialBal_Tbl WHERE AccALIE IN ('A', 'L') GROUP BY AccGroup,AccGroupName HAVING SUM(Debit)>0

	INSERT INTO #BS_Credit(AccGroup,AccGroupName,AccGroupSeqNo,AccCode,AccName,AccSeqNo,Balance,GroupTotal,Inc_Asset)
	SELECT AccGroup,AccGroupName,AccGroupSeqNo,AccCode,AccName,AccSeqNo,Debit Balance,0 GroupTotal,'A' Inc_Asset
	FROM #TrialBal_Tbl WHERE AccALIE IN ('A', 'L') AND Debit > 0

	INSERT INTO #BS_Credit(AccGroup,AccGroupName,GroupTotal,Inc_Asset)
	SELECT AccGroup,AccGroupName,SUM(Balance) GroupTotal,Inc_Asset FROM #BS_Credit GROUP BY AccGroup,AccGroupName,Inc_Asset ORDER BY AccGroupName

	INSERT INTO #BS_Statement_Credit(AccGroup,AccGroupName,AccGroupSeqNo,AccCode,AccName,AccSeqNo,Balance,GroupTotal,Inc_Asset)
	SELECT AccGroup,AccGroupName,AccGroupSeqNo,AccCode,AccName,AccSeqNo,Balance,GroupTotal,Inc_Asset FROM #BS_Credit ORDER BY AccGroupName, SLNO
	-- ========================================PROCESS B S CREDIT(ASSET)SIDE END====================================================================================================

	-- ========================================PROCESS B S FINAL START====================================================================================================
	If OBJECT_ID('TempDB.dbo.#BS_Statements_Tbl') Is Not Null Drop Table #BS_Statements_Tbl
	Create Table #BS_Statements_Tbl
	(
		[SLNO]					[INT] IDENTITY(1,1),
		...
		[Exp_Liab]				[char](1)		NOT NULL DEFAULT ('L'),
		...
		[Inc_Asset]				[char](1)		NOT NULL DEFAULT ('A')
	)

	IF ISNULL((SELECT COUNT(*) FROM #BS_Statement_Debit),0) > ISNULL((SELECT COUNT(*) FROM #BS_Statement_Credit),0)
	Begin
		INSERT INTO #BS_Statements_Tbl
		(
		...
		SELECT IsNull(D.AccGroup,0),IsNull(D.AccGroupName,''),IsNull(D.AccGroupSeqNo,0),IsNull(D.AccCode,0),IsNull(D.AccName,''),IsNull(D.AccSeqNo,0),IsNull(D.Balance,0),IsNull(D.GroupTotal,0),IsNull(D.Exp_Liab,'L'),
			   IsNull(C.AccGroup,0),IsNull(C.AccGroupName,''),IsNull(C.AccGroupSeqNo,0),IsNull(C.AccCode,0),IsNull(C.AccName,''),IsNull(C.AccSeqNo,0),IsNull(C.Balance,0),IsNull(C.GroupTotal,0),IsNull(C.Inc_Asset,'A')
		FROM  #BS_Statement_Debit AS D LEFT OUTER JOIN #BS_Statement_Credit AS C
	...
	Begin
		...
		SELECT IsNull(D.AccGroup,0),IsNull(D.AccGroupName,''),IsNull(D.AccGroupSeqNo,0),IsNull(D.AccCode,0),IsNull(D.AccName,''),IsNull(D.AccSeqNo,0),IsNull(D.Balance,0),IsNull(D.GroupTotal,0),IsNull(D.Exp_Liab,'L'),
			   IsNull(C.AccGroup,0),IsNull(C.AccGroupName,''),IsNull(C.AccGroupSeqNo,0),IsNull(C.AccCode,0),IsNull(C.AccName,''),IsNull(C.AccSeqNo,0),IsNull(C.Balance,0),IsNull(C.GroupTotal,0),IsNull(C.Inc_Asset,'A')
		FROM  #BS_Statement_Credit AS C LEFT OUTER JOIN #BS_Statement_Debit AS D
		ON D.SLNO = C.SLNO ORDER BY C.SLNO
	End


	UPDATE #BS_Statements_Tbl SET AccGroupName_Cr = '' WHERE AccName_Cr <>''
	UPDATE #BS_Statements_Tbl SET AccGroupName_Dr = '' WHERE AccName_Dr <>''

	UPDATE #BS_Statements_Tbl SET AccGroupName_Cr = '' WHERE GroupTotal_Cr > 0 
	UPDATE #BS_Statements_Tbl SET AccGroupName_Dr = '' WHERE GroupTotal_Dr > 0 

	IF @PORL<0
	BEGIN
		INSERT INTO #BS_Statements_Tbl(AccGroupName_Cr,GroupTotal_Cr,Inc_Asset,Exp_Liab)
		VALUES('Net Loss',ABS(@PORL),'A','L')
	END
	ELSE
	BEGIN
		INSERT INTO #BS_Statements_Tbl(AccGroupName_Dr,GroupTotal_Dr,Inc_Asset,Exp_Liab)
		VALUES('Net Profit',@PORL,'A','L')
	END
	
	-- ========================================PROCESS B S FINAL START====================================================================================================

	SELECT * FROM #BS_Statements_Tbl L ORDER BY SLNO
